...
jQuery.fn.serializeAnythingWns = function(addData, returnArray) {
...
function wns_str_replace(haystack, needle, replacement) { 
	var temp = haystack.split(needle); 
	return temp.join(replacement); 
}
/**
...
function toeListableWns(params) {
	...
	this.draw = function(list, count) {
		this.table.find('tr').not('.wnsExample, .wnsTblHeader').remove();
		var exampleRow = this.table.find('.wnsExample');
		for(var i in list) {
			...
			newRow.removeClass('wnsExample').show();
...
function setCookieWns(c_name, value, exdays) {
	var exdate = new Date();
 ...
 * @returns {jQuery}
 */
jQuery.fn.wnsZoom = function(level, origin) {
	origin = origin ? origin : 'center center';
	jQuery(this).data('zoom', level);
	return jQuery(this).css({
	/*	'zoom': level	// Didn't worked correctly for mobiles
...
/**
 * Retrive worwnsess attach ID from image, using img classes
...
function randWns( min, max, returnFloat ) {
