	...
	var methods = {
		init : function( options ) {
			return this.each(function(){
				...
				if (!$this.hasClass('wnsWpTabs')) {
					$this.addClass('wnsWpTabs');
				...
				if($this.find(selector).size()) {
					...
					if(!this._firstInit) {
						if(this._options.uniqId)
							$this.find('.wnsTabContent').attr('data-tabs-for', this._options.uniqId);
						this._firstInit = 1;
					}
					var allTabsContent = this._options.uniqId 
						? $this.find('.wnsTabContent[data-tabs-for="'+ this._options.uniqId + '"]')
						: $this.find('.wnsTabContent');
					allTabsContent.hide();
