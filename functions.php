 ...
 */
if(!function_exists('dateToTimestampWns')) {
    function dateToTimestampWns($date) {
        if(empty($a))
            return false;
        $a = explode(WNS_DATE_DL, $date);
...
if(!function_exists('importWns')) {
    function importWns($path) {
        if(file_exists($path)) {
            require($path);
            return true;
        }
...
if(!function_exists('importClassWns')) {
    function importClassWns($class, $path = '') {
        if(!class_exists($class)) {
            if(!$path) {
				$classFile = $class;
				if(strpos(strtolower($classFile), WNS_CODE) !== false) {
					$classFile = preg_replace('/'. WNS_CODE. '/i', '', $classFile);
				}
                $path = WNS_CLASSES_DIR. $classFile. '.php';
			}
            return importWns($path);
            ...
            /*if(class_exists('ReflectionClass')) {   //ReflectionClass supported begining from php5
                ...
                if(strpos($classFile, WNS_DIR) === false) {   //Class is not in our plugin directory
                    ...
                    if(!empty($plugins)) {
                        for($i = 0; $i < count($plugins); $i++) {
                            if(strpos($plugins[$i], WNS_PLUG_NAME) !== false) {   //Let's remove our plugin from list of active plugins
                                unset($plugins[$i]);
                            }
                        }
 ...
 */
if(!function_exists('toeGetClassNameWns')) {
    function toeGetClassNameWns($class) {
        $className = '';
		if(class_exists($class. strFirstUp(WNS_CODE))) 
			$className = $class. strFirstUp(WNS_CODE);
		else if(class_exists(WNS_CLASS_PREFIX. $class)) 
            $className = WNS_CLASS_PREFIX. $class;
		else
            $className = $class;
        return $className;
    } 
 ...
 */
if(!function_exists('toeCreateObjWns')) {
    function toeCreateObjWns($class, $params) {
        $className = toeGetClassNameWns($class);
        $obj = NULL;
 ...
 */
if(!function_exists('redirectWns')) {
    function redirectWns($url) {
        if(headers_sent()) {
 ...
 * @return array
 */
if(!function_exists('prepareParamsWns')) {
	function prepareParamsWns(&$d=array(), &$options = array()) {
		if (!empty($d['params'])) {
			...
			if (is_array($d['params'])) {
				$params = utilsWns::jsonEncode($d['params']);
		...
		if(isset($d['code'])) {
			if ($d['code'] == '') {
				$d['code'] = prepareFieldCodeWns($d['label']).'_'.rand(0, 9999999);
			}
		}
		return $d;
	}
}
if(!function_exists('prepareFieldCodeWns')) {
	function prepareFieldCodeWns($string) {   
