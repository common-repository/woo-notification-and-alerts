<section class="supsystic-bar">
	<ul class="supsystic-bar-controls">
		<li title="<?php _e('Save all options')?>">
			<button class="button button-primary" id="wnsSettingsSaveBtn" data-toolbar-button>
...
<section>
	<form id="wnsSettingsForm" class="wnsInputsWithDescrForm">
							...
							<?php foreach($optCatData['opts'] as $optKey => $opt) { ?>
								<?php
									...
									if(isset($opt['pro']) && !empty($opt['pro'])) {
										$htmlOpts['attrs'] .= ' class="wnsProOpt"';
									}
								?>
								<tr
								...
								>
									<th scope="row" class="col-w-30perc">
										...
										<?php if(isset($opt['pro']) && !empty($opt['pro'])) { ?>
											<span class="wnsProOptMiniLabel">
												<a href="<?php echo $opt['pro']?>" target="_blank">
													<?php _e('PRO option', WNS_LANG_CODE)?>
												</a>
											</span>
										<?php }?>
									</th>
									...
									<td class="col-w-1perc">
										<?php echo htmlWns::$htmlType('opt_values['. $optKey. ']', $htmlOpts)?>
									</td>
									<td class="col-w-60perc">
										<div id="wnsFormOptDetails_<?php echo $optKey?>" class="wnsOptDetailsShell">
										<?php switch($optKey) {
		...
		<?php echo htmlWns::hidden('mod', array('value' => 'options'))?>
		<?php echo htmlWns::hidden('action', array('value' => 'saveGroup'))?>
	</form>
	<br />
</section>