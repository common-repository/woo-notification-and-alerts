<?php
class promoModelWns extends modelWns {
	private $_apiUrl = '';
	...
	public function saveUsageStat($code, $unique = false) {
		...
		$query = 'INSERT INTO @__usage_stat SET code = "'. dbWns::escape($code). '", visits = 1
			ON DUPLICATE KEY UPDATE visits = visits + 1';
		return dbWns::query($query);
	}
	private function _checkUniqueStat($code) {
		$uniqueStats = get_option(WNS_CODE. '_unique_stats');
		...
		$uniqueStats[] = $code;
		update_option(WNS_CODE. '_unique_stats', $uniqueStats);
		return false;
	}
	...
	public function getUserStatsCount() {
		...
		return (int) dbWns::get($query, 'one');
	}
	...
	public function getTourHst() {
		$hst = get_user_meta(get_current_user_id(), WNS_CODE . '-tour-hst', true);
		if(empty($hst))
			$hst = array();
		if(!isset($hst['passed']))
			$hst['passed'] = array();
	...
	public function setTourHst( $hst ) {
		update_user_meta(get_current_user_id(), WNS_CODE . '-tour-hst', $hst);
	}
	public function clearTourHst() {
		delete_user_meta(get_current_user_id(), WNS_CODE . '-tour-hst');
	}
	...
	private function _getBigStatClient() {
		if(!$this->_bigCli) {
			...
			if(class_exists('Mixpanel')) {
				...
				/*$bigCliId = (int) get_option(WNS_CODE. '_big_cli_id');
				if(!$bigCliId) {
					$bigCliId = mt_rand(1, 9999999999);
					update_option(WNS_CODE. '_big_cli_id', $bigCliId);
				}
	...
	public function saveDeactivateData( $d ) {
		...
		$startUsage = (int) frameWns::_()->getModule('options')->get('plug_welcome_show');
		if($startUsage) {
