...
class langWns {
	...
	static public function extract($d = array('dir' => '', 'langWns' => '')) {
		$data = array();
		if(isset($d['dir']) && !empty($d['dir']))
			$langDirPath = $d['dir'];
		else if(isset($d['langWns']) && !empty($d['langWns']))
			$langDirPath = WNS_LANG_DIR. $d['langWns']. DS;
		else
			$langDirPath = WNS_LANG_DIR. WNS_WPLANG. DS;
