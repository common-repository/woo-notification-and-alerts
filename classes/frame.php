...
class frameWns {
    ...
    public function __construct() {
        $this->_res = toeCreateObjWns('response', array());

    }
    static public function getInstance() {
        static $instance;
        if(!$instance) {
            $instance = new frameWns();
        }
        return $instance;
    }
	...
	private function _checkPromoModName($activeModules) {
		foreach($activeModules as $i => $m) {
			if($m['code'] == 'supsystic_promo') {	// Well, rename it ;)
				...
				dbWns::query("UPDATE `@__modules` SET code = 'promo', label = 'promo' WHERE code = 'supsystic_promo'");
			}
		}
        ...
        if($activeModules) {
            foreach($activeModules as $m) {
                ...
                if(!empty($m['ex_plug_dir'])) {
                    $moduleLocationDir = utilsWns::getExtModDir( $m['ex_plug_dir'] );
                }
                if(is_dir($moduleLocationDir. $code)) {
                    $this->_allModules[$m['code']] = 1;
                    if((bool)$m['active']) {
                        importClassWns($code. strFirstUp(WNS_CODE), $moduleLocationDir. $code. DS. 'mod.php');
                        $moduleClass = toeGetClassNameWns($code);
    ...
    public function init() {
		...
		dispatcherWns::doAction('afterModulesInit');

		modInstallerWns::checkActivationMessages();
		...
		global $langOK;
        register_activation_hook(  WNS_DIR. DS. WNS_MAIN_FILE, array('utilsWns', 'activatePlugin')  ); //See classes/install.php file
        register_uninstall_hook(WNS_DIR. DS. WNS_MAIN_FILE, array('utilsWns', 'deletePlugin'));
		register_deactivation_hook(WNS_DIR. DS. WNS_MAIN_FILE, array( 'utilsWns', 'deactivatePlugin' ) );
	...
	public function connectLang() {
		global $langOK;
		$langOK = load_plugin_textdomain(WNS_LANG_CODE, false, WNS_PLUG_NAME. '/languages/');
	}
    /**
    ...
    public function checkPermissions($code, $action) {
        if($this->havePermissions($code, $action))
            ...
            exit(_e('You have no permissions to view this page', WNS_LANG_CODE));
        }
    }
    /**
    ...
    public function havePermissions($code, $action) {
        ...
        if($mod) {
            ...
            if(!empty($permissions)) {  // Special permissions
                if(isset($permissions[WNS_METHODS])
                    && !empty($permissions[WNS_METHODS])
                ) {
                    foreach($permissions[WNS_METHODS] as $method => $permissions) {   // Make case-insensitive
                        $permissions[WNS_METHODS][strtolower($method)] = $permissions;
                    }
                    if(array_key_exists($action, $permissions[WNS_METHODS])) {        // Permission for this method exists
                        $currentUserPosition = frameWns::_()->getModule('user')->getCurrentUserPosition();
                        if((is_array($permissions[ WNS_METHODS ][ $action ]) && !in_array($currentUserPosition, $permissions[ WNS_METHODS ][ $action ]))
                            || (!is_array($permissions[ WNS_METHODS ][ $action ]) && $permissions[WNS_METHODS][$action] != $currentUserPosition)
                        ) {
                            $res = false;
                        }
                    }
                }
                if(isset($permissions[WNS_USERLEVELS])
                    && !empty($permissions[WNS_USERLEVELS])
                ) {
                    $currentUserPosition = frameWns::_()->getModule('user')->getCurrentUserPosition();
					// For multi-sites network admin role is undefined, let's do this here
					if(is_multisite() && is_admin() && is_super_admin()) {
						$currentUserPosition = WNS_ADMIN;
					}
                    foreach($permissions[WNS_USERLEVELS] as $userlevel => $methods) {
                        if(is_array($methods)) {
    ...
    protected function _extractTables($tablesDir = WNS_TABLES_DIR) {
    ...
    protected function _extractTable($tableName, $tablesDir = WNS_TABLES_DIR) {
        importClassWns('noClassNameHere', $tablesDir. $tableName. '.php');
        $this->_tables[$tableName] = tableWns::_($tableName);
    }
    /**
     ...
     * @return object table
     * @example frameWns::_()->getTable('products')->getAll()
     */
    ...
    public function addScript($handle, $src = '', $deps = array(), $ver = false, $in_footer = false, $vars = array()) {
		$src = empty($src) ? $src : uriWns::_($src);
		if(!$ver)
			$ver = WNS_VERSION;
    ...
    /**
     * Add all scripts from _scripts array to worwnsess
     */
    ...
    public function isTplEditor() {
        $tplEditor = reqWns::getVar('tplEditor');
	...
	public function isAdminPlugOptsPage() {
		$page = reqWns::getVar('page');
		if(is_admin() && !empty($page) && strpos($page, frameWns::_()->getModule('adminmenu')->getMainSlug()) !== false)
			return true;
		return false;
	}
	...
	public function savePluginActivationErrors() {
		update_option(WNS_CODE. '_plugin_activation_errors',  ob_get_contents());
	}
	public function getActivationErrors() {
		return get_option(WNS_CODE. '_plugin_activation_errors');
	}
