...
abstract class tableWns {
    ...
    static public function getInstance($table = '') {
        ...
        if(!isset($instances[$table])) {
            $class = 'table'. strFirstUp($table). strFirstUp(WNS_CODE);
            ...
            if($id) {
                ...
                foreach($data as $field) {
                    ...
                    foreach($field as $k => $v) {
                        if(isset($this->_fields[$k])) {
                            $row[$k] = toeCreateObjWns('fieldWns', array(
                                    $this->_fields[$k]->name,
                                    $this->_fields[$k]->html,
                                    $this->_fields[$k]->type,
    ...
    public function getTable($transform = false) {
        if($transform)
            return dbWns::prepareQuery($this->_table);
        else
            return $this->_table;
    }
    ...
    public function store($data, $method = 'INSERT', $where = '') {
        ...
        if(empty($fields)) {
            $this->_addError(__('Nothing to update', WNS_LANG_CODE));
            return false;
        }
        ...
        if(dbWns::query($query)) {
			...
			$this->_addError(WNS_TEST_MODE ? dbWns::getError() : __('Database error. Please contact your developer.', WNS_LANG_CODE));
        return false;
    }
    ...
    /**
     * Add new fieldWnsWns for children table (@see class field)
    ...
    protected function _addField($name, $html = 'text', $type = 'other', $default = '', $label = '', $maxlen = 0, $dbAdapt = '', $htmlAdapt = '', $description = '') {
        $this->_fields[$name] = toeCreateObjWns('fieldWns', array($name, $html, $type, $default, $label, $maxlen, $dbAdapt, $htmlAdapt, $description));
        return $this;
    }
	/**
    ...
    public function exists($value, $field = '') {
        if(!$field)
            $field = $this->_id;
        return dbWns::get('SELECT '. $this->_id. ' FROM '. $this->_table. ' WHERE '. $field. ' = "'. $value. '"', 'one');
    }
            ...
            switch($f->type) {
                case 'date':
                    if($d[$key] == '0000-00-00' || empty($d[$key]))
                        ...
                        $d[$key] = date(WNS_DATE_FORMAT, dbWns::dateToTime($d[$key]));
                    }
                    break;
                case 'int':
                case 'tinyint':
    ...
    public function getLastInsertID() {
        return dbWns::get('SELECT MAX('. $this->_id. ') FROM '. $this->_table, 'one');
    }
