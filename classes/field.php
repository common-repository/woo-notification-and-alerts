...
class fieldWns {
	 ...
	 */
	public $adapt = array('htmlWns' => '', 'dbFrom' => '', 'dbTo' => '');
	/**
	...
	public function setValue($value, $fromDB = false) {
		if(isset($this->adapt['dbFrom']) && $this->adapt['dbFrom'] && $fromDB)
			$value = fieldAdapterWns::_($value, $this->adapt['dbFrom'], fieldAdapterWns::DB);
		$this->value = $value;
	}
	...
	public function drawHtml($tag, $id) {
		if(method_exists('htmlWns', $this->html)) {
			$method = $this->html;
			...
			if($this->adapt['html']) {
				fieldAdapterWns::_($this, $this->adapt['html'], fieldAdapterWns::HTML);
			}
			$params = $this->processParams($tag, $id);
			if ($params != '')
				return $params;
			if ($this->name == 'default_value') {
				$optionsFromDb = frameWns::_()->getModule('optionsWns')->getHelper()->getOptions($id);
				if (!empty($optionsFromDb)) {
					$options = array(0 => __('Select', WNS_LANG_CODE));
					foreach($optionsFromDb as $k => $v)
						$options[$k] = $v;
					$method = 'selectbox';
					$this->htmlParams['optionsWns'] = $options;
				}
			}
			$htmlContent = htmlWns::$method($this->name, $this->htmlParams);
			if(!empty($this->errorEl)) {
				if(is_bool($this->errorEl))
					$errorEl = '<div class="toeErrorForField toe_'. htmlWns::nameToClassId($this->name). '"></div>';
	...
	public function displayValue() {
		...
		switch($this->html) {
			case 'countryList':
				$value = fieldAdapterWns::displayCountry($this->value);
				break;
			case 'statesList':
				$value = fieldAdapterWns::displayState($this->value);
				break;
			case 'checkboxlist':
				$options = $this->getHtmlParam('optionsWns');
				$value = array();
	...
	public function viewField($name, $fieldValue = '') {
		$method = $this->html;
		$options = frameWns::_()->getModule('optionsWns')->getHelper()->getOptions($this->id);
		$attrs = '';
		...
		$params = array('optionsWns'=>$options, 'attrs' => $attrs, 'value' => $fieldValue, 'checked' => $checked);
		$output = '';
		if(method_exists('htmlWns', $method)) {
			$output .= htmlWns::$method($name, $params);
			$output .= htmlWns::hidden('extra_field['.$this->name.']',array('value'=>$this->id));
		}
		return $output;
	}
	...
	public function processParams($tag, $id){
		...
		if ($this->name == "params") {
			...
			switch ($tag) {
				case 5: 
					$add_option = __('Add Checkbox', WNS_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 9: 
					$add_option = __('Add Item', WNS_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 12:
				...
				case 10:
					$options_tag = '';
					$add_option = __('Add Radio Button', WNS_LANG_CODE);
					$image_tag = ' style="display:none"';
				break;
				case 8:
					$image_tag = '';
			...
			if ($tag > 0 || $id == 0) {
				$output .= '<div class="options options_tag"'.$options_tag.'>';
					...
					$output .= fieldAdapterWns::_($id,'getExtraFieldOptions',fieldAdapterWns::STR);
				$output .= '</div>';

				$output .= '<div class="options image_tag"'.$image_tag.'>'.__('Dimensions', WNS_LANG_CODE).':<br />';
					$params->width?$width = $params->width:'';
					$params->height?$height = $params->height:'';
					$output .= __('width', WNS_LANG_CODE).':<br />';
					$output .= htmlWns::text('params[width]',array('value'=>$width)).'<br />';
					$output .= __('height', WNS_LANG_CODE).':<br />';
					$output .= htmlWns::text('params[height]',array('value'=>$height)).'<br />';
				$output .= '</div>';
			}
			if($this->adapt['htmlParams']) {
				$output .= fieldAdapterWns::_($this, $this->adapt['htmlParams'], fieldAdapterWns::STR);
			} else {
				$output .= '<a href="javascript:void(0);" class="set_properties">'.__('Click to set field "id" and "class"', WNS_LANG_CODE).'</a>';
				$output .= '<div class="attributes" style="display:none;">'.__('Attributes', WNS_LANG_CODE).':<br />';
				$output .= fieldAdapterWns::_($params,'getFieldAttributes',  fieldAdapterWns::STR);
				$output .= '</div>';
			}
			return $output;
		}
	...
	function checkVarFromParam($param, $element) {
		return utilsWns::xmlAttrToStr($param, $element);
	  ...
	  // load xml structure of parameters
	   ...
	   foreach ($config->params->param as $param) {
		 // read the variables
		  ...
		  $helper = $this->checkVarFromParam($param,'helperWns');
		  $module = $this->checkVarFromParam($param,'moduleWns');
	...
	public function drawConfig($module, $additionalOptions = array()) {
		if(!frameWns::_()->getModule($module)) 
			return false; 
		// check for xml file with params structure  
	   if(frameWns::_()->getModule($module)->isExternal())
		   $config_xml = frameWns::_()->getModule($module)->getModDir(). 'mod.xml';
	   else
		   $config_xml = WNS_MODULES_DIR.$module.DS.'mod.xml';

	   if (!file_exists($config_xml)) {
		   // if there is no configuration file for this $module
		   return __('There are no configuration options for this module', WNS_LANG_CODE);
	   }
	   $output = '';
		   ...
		   foreach ($params as $param) {
			   ...
			   foreach ($configOptions as $key=>$value){
				  ...
				  // if helper is needed to render the object
					  ...
					  $helper = frameWns::_()->getModule($hmodule)->getHelper();
					  if ($helper) {
				  ...
				  // filling the parameters to build html element
					 $htmlParams = array('value'=>$fieldValue,'optionsWns'=>$options);
					 if($method == 'checkbox') {
						 $htmlParams['value'] = 1;
				  ...
				  // output label and html element
					 ...
					 $output .= '</label><br />';
					 $output .= htmlWns::$method($name,$htmlParams).'<br />';
					 $output .= '</div>';
			   }
			   $i++;
			 $output .= '</div>';
