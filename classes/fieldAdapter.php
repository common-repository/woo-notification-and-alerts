...
class fieldAdapterWns {
    ...
    static public function _($fieldOrValue, $method, $type) {
        if(method_exists('fieldAdapterWns', $method)) {
            switch($type) {
                case self::DB:
    ...
    static public function userFieldDestHtml($field) {
        $field->htmlParams['optionsWns'] = array();
    ...
    static public function userFieldDestToDB($value) {
        return utilsWns::jsonEncode($value);
    }
    static public function userFieldDestFromDB($value) {
        return utilsWns::jsonDecode($value);
    }
    static public function taxDataHtml($field) {
        ...
        $categories = frameWns::_()->getModule('products')->getCategories();
        $brands = frameWns::_()->getModule('products')->getBrands();
        $cOptions = array();
        $bOptions = array();
        if(!empty($categories)) {
        ...
        return '<div>'. __('Apply To', WNS_LANG_CODE). '
            <div id="tax_address">
                <b>'. __('Address', WNS_LANG_CODE). '</b><br />
                '. __('Destination', WNS_LANG_CODE). ':'. htmlWns::checkboxlist('params[dest]', array('optionsWns' => $listOfDest)). '<br />
                '. __('Country', WNS_LANG_CODE). ':'. htmlWns::countryList('params[country]', array('notSelected' => true, 'value' => $field->value['country'])). '<br />
            </div>
            <div id="tax_category">
                <b>'. __('Categories', WNS_LANG_CODE). '</b><br />
                '. (empty($cOptions) ? __('You have no categories', WNS_LANG_CODE) : htmlWns::checkboxlist('params[categories][]', array('optionsWns' => $cOptions))). '<br />
                    <b>'. __('Brands', WNS_LANG_CODE). '</b><br />
                '. (empty($bOptions) ? __('You have no brands', WNS_LANG_CODE) : htmlWns::checkboxlist('params[brands][]', array('optionsWns' => $bOptions))). '<br />
            </div>
            <div>'. __('Tax Rate', WNS_LANG_CODE).': '. htmlWns::text('params[rate]', array('value' => $field->value['rate'])).'</div>
            <div>'. __('Absolute', WNS_LANG_CODE).': '. htmlWns::checkbox('params[absolute]', array('checked' => $field->value['absolute'])).'</div>
        </div>';
    }
    ...
    static public function getCountries($notSelected = false) {
        ...
        if(empty($options[ $notSelected ])) {
            ...
            if(empty(self::$countries))
                self::$countries = self::getCachedCountries();
            if($notSelected) {
				$options[ $notSelected ][0] = is_bool($notSelected) ? __('Not selected', WNS_LANG_CODE) : __($notSelected);
			}
    ...
    static public function getExtraFieldOptions($field_id) {
        ...
        $options = frameWns::_()->getModule('optionsWns')->getHelper()->getOptions($field_id);
        if (!empty($options)) {
    ...
    static public function getFieldAttributes($params){
        ...
        if (!empty($params->attr)) {
            foreach ($params->attr as $key=>$value) {
                $output .= __($key).':<br />';
                $output .= htmlWns::text('params[attr]['.$key.']',array('value'=>$value)).'<br />';
            }
        } else {
                $output .= __('class', WNS_LANG_CODE).':<br />';
                $output .= htmlWns::text('params[attr][class]',array('value'=>'')).'<br />';
                $output .= __('id', WNS_LANG_CODE).':<br />';
                $output .= htmlWns::text('params[attr][id]',array('value'=>'')).'<br />';
        }
        return $output;
    }
    /**
    ...
    static function productFieldCategories($field){
        if(!empty($field->htmlParams['optionsWns']))
            return;
        ...
        if(!empty($categories)) {
            if(!is_array($field->value['categories']))
                    $field->value['categories'] = array();
            $field->htmlParams['optionsWns'][0] = in_array(0,$field->value['categories'])?__('Deselect All'):__('Select All', WNS_LANG_CODE);
            foreach($categories as $c) {
                $field->htmlParams['optionsWns'][$c->term_taxonomy_id] = $c->cat_name;
            }
        }*/
    }
	...
	static public function getCachedCountries($clearCache = false) {
		if(empty(self::$countries) || $clearCache)
			self::$countries = frameWns::_()->getTable('countries')->getAll('id, name, iso_code_2, iso_code_3');
	...
	static public function getCachedStates($clearCache = false) {
		if(empty(self::$states) || $clearCache)
			self::$states = frameWns::_()->getTable('states')
				->leftJoin( frameWns::_()->getTable('countries'), 'country_id' )
