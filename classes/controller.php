...
abstract class controllerWns {
	...
	protected function _createModel($name = '') {
		if(empty($name)) $name = $this->getCode();
		$parentModule = frameWns::_()->getModule( $this->getCode() );
		$className = '';
		if(importWns($parentModule->getModDir(). 'models'. DS. $name. '.php')) {
			$className = toeGetClassNameWns($name. 'Model');
		}
	...
	protected function _createView($name = '') {
		...
		if(importWns($parentModule->getModDir(). 'views'. DS. $name. '.php')) {
			$className = toeGetClassNameWns($name. 'View');
		}
	 ...
	 * @example :
	 return array(
			S_METHODS => array(
				'save' => array(WNS_ADMIN),
				'remove' => array(WNS_ADMIN),
				'restore' => WNS_ADMIN,
			),
			S_USERLEVELS => array(
	 ...
	 * Can be used on of sub-array - WNS_METHODS or WNS_USERLEVELS
	 */
	...
	public function getModule() {
		return frameWns::_()->getModule( $this->getCode() );
	}
	...
	public function getListForTbl() {
		...
		$model = $this->getModel();
		$page = (int) reqWns::getVar('page');
		$rowsLimit = (int) reqWns::getVar('rows');
		$orderBy = reqWns::getVar('sidx');
		$sortOrder = reqWns::getVar('sord');
		// Our custom search
		$search = reqWns::getVar('search');
		...
		// jqGrid search
		$isSearch = reqWns::getVar('_search');
		if($isSearch) {
			$searchField = trim(reqWns::getVar('searchField'));
			$searchString = trim(reqWns::getVar('searchString'));
		...
		$res = dispatcherWns::applyFilters($this->getCode(). '_getListForTblResults', $res);
		$res->ajaxExec();

	}
	public function removeGroup() {
		$res = new responseWns();
		if($this->getModel()->removeGroup(reqWns::getVar('listIds', 'post'))) {
