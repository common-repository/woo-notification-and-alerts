...
class utilsWns {
    ...
    static public function copyDirectories($source, $destination) {
        if(is_dir($source)) {
            ...
            while ( FALSE !== ( $readdirectory = $directory->read() ) ) {
                ...
                if (is_dir($PathDir)) {
                    utilsWns::copyDirectories( $PathDir, $destination . '/' . $readdirectory );
                    continue;
                }
    ...
    static public function xmlNodeAttrsToArr($node) {
        ...
        foreach($node->attributes() as $a => $b) {
            $arr[$a] = utilsWns::xmlAttrToStr($node, $a);
        }
        return $arr;
    }
    ...
    static public function isThisCommercialEdition() {
        ...
        foreach($commercialModules as $m) {
            if(!frameWns::_()->getModule($m)) 
                return false;
            if(!is_dir(frameWns::_()->getModule($m)->getModDir())) 
                return false;
        }
        return true;*/
        foreach(frameWns::_()->getModules() as $m) {
	...
	static public function isMobile() {
		importClassWns('Mobile_Detect', WNS_HELPERS_DIR. 'mobileDetect.php');
	...
	static public function isPluginsPage() {
		return (basename(reqWns::getVar('SCRIPT_NAME', 'server')) === 'plugins.php');
	}
	...
	/**
	 * Parse worwnsess post/page/custom post type content for images and return it's IDs if there are images
	...
	static public function getReferalUrl() {
		// Simple for now
		return reqWns::getVar('HTTP_REFERER', 'server');
	}
