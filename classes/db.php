...
class dbWns {
     ...
     * #__  Worwnsess prefix
     * ^__  Store plugin tables prefix (@see WNS_DB_PREF if config.php)
    ...
    static public function prepareQuery($query) {
        ...
        return str_replace(
                array('#__', '^__', '@__'),
                array($wpdb->prefix, WNS_DB_PREF, $wpdb->prefix. WNS_DB_PREF),
                $query);
    }
    ...
    static public function timeToDate($timestamp = 0) {
        if($timestamp) {
            if(!is_numeric($timestamp))
                $timestamp = dateToTimestampWns($timestamp);
    ...
    static public function dateToTime($date) {
        ...
        if(strpos($date, WNS_DATE_DL)) return dateToTimestampWns($date);
        $arr = explode('-', $date);
        return dateToTimestampWns($arr[2]. WNS_DATE_DL. $arr[1]. WNS_DATE_DL. $arr[0]);
    }
